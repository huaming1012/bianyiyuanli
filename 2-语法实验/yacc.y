...
CompUnit: Decl{$$ = TreeNode1("CompUnit", 0); addChild($$, $1); tree = $$;
}
|FuncDef{$$ = TreeNode1("CompUnit", 0); addChild($$, $1);tree = $$; 
}
|CompUnit Decl{$$ = TreeNode1("CompUnit", 0); addChild($$, $1); addChild($$, $2); tree = $$;
}
|CompUnit FuncDef{$$ = TreeNode1("CompUnit", 0); addChild($$, $1); addChild($$, $2);tree = $$; 
}
;
...
|IF LP Cond RP Stmt %prec IFX{$$ = TreeNode1("Stmt", 0);
TreeNode* tmp1 = TreeNode1("if", 1);
TreeNode* tmp2 = TreeNode1("\\(", 1);
TreeNode* tmp3 = TreeNode1("\\)", 1);
addChild($$, tmp1);
